...
function leadCard(lead) {
  ...
  return `<article class="lead-card ${lead.contacted_at ? "lead-contacted" : ""}" draggable="true" data-action="edit-lead" data-id="${lead.id}" title="Arraste para mudar de etapa ou clique para editar"><div class="lead-card-top"><div class="lead-card-selection-wrap">${selectAction}<div><h3 title="${escapeHtml(fullCompanyName)}"><span class="lead-card-title-full">${escapeHtml(fullCompanyName)}</span><span class="lead-card-title-mobile">${escapeHtml(visibleCompanyName)}</span></h3><div class="lead-card-subtitle"></div></div></div><div class="lead-card-header-actions"><span class="lead-card-value">${formatCurrency(lead.deal_value || state.settings.deal_value)}</span>${deleteAction}</div></div><div class="lead-meta">${followUpIndicator}</div>${activityMarkup}<div class="lead-card-footer">${whatsappAction}${activityAction}</div></article>`;
}
...
function addMessageTemplateRow() {
  ...
  const stage = $('#message-settings-form')?.dataset.messageStage === "offer" ? "Proposta" : "Saudação";
